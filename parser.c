...
{
    m->no_t.img = mlx_xpm_file_to_image(m->mlx, m->no, &m->no_t.width, &m->no_t.height);
    if (m->no_t.img == NULL)
        return (-13);
    m->no_t.addr = mlx_get_data_addr(m->no_t.img, &m->no_t.bpp, &m->no_t.ll, &m->no_t.end);
    m->so_t.img = mlx_xpm_file_to_image(m->mlx, m->so, &m->so_t.width, &m->so_t.height);
    if (m->so_t.img == NULL)
        return (-13);
    m->so_t.addr = mlx_get_data_addr(m->so_t.img, &m->so_t.bpp, &m->so_t.ll, &m->so_t.end);
    m->ea_t.img = mlx_xpm_file_to_image(m->mlx, m->ea, &m->ea_t.width, &m->ea_t.height);
    if (m->ea_t.img == NULL)
        return (-13);
    m->ea_t.addr = mlx_get_data_addr(m->ea_t.img, &m->ea_t.bpp, &m->ea_t.ll, &m->ea_t.end);
    m->we_t.img = mlx_xpm_file_to_image(m->mlx, m->we, &m->we_t.width, &m->we_t.height);
    if (m->we_t.img == NULL)
        return (-13);
    m->we_t.addr = mlx_get_data_addr(m->we_t.img, &m->we_t.bpp, &m->we_t.ll, &m->we_t.end);
    return (0);
}
